...
from src.stage2.parser.parser_Embedding_based import *
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        if train_method == "Multitask":
            ...
            for iter in range(1, n_batch + 1):
                ...
                batch_loss = model(cf_batch_user, cf_batch_pos_item, cf_batch_neg_item, kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail, mode='train_cf&kg')
                ...
                if (iter % args.print_every) == 0:
                    logging.info(
                        'CF Training: Epoch {:04d} Iter {:04d} / {:04d} | Time {:.1f}s | Iter Loss {:.4f} | Iter Mean Loss {:.4f}'.format(
                            epoch, iter, n_cf_batch, time() - time4, cf_batch_loss.item(), cf_total_loss / iter))
            logging.info(
                ...
                if (iter % args.print_every) == 0:
                    logging.info(
                        'KG Training: Epoch {:04d} Iter {:04d} / {:04d} | Time {:.1f}s | Iter Loss {:.4f} | Iter Mean Loss {:.4f}'.format(
                            epoch, iter, n_kg_batch, time() - time6, kg_batch_loss.item(), kg_total_loss / iter))
            logging.info(
