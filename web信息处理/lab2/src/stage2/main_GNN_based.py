...
from src.stage2.parser.parser_GNN_based import *
        ...
        if train_method == "Iteration":
            ...
            for iter in range(1, n_cf_batch + 1):
                ...
                if np.isnan(cf_batch_loss.cpu().detach().numpy()):
                    logging.info(
                        'ERROR (CF Training): Epoch {:04d} Iter {:04d} / {:04d} Loss is nan.'.format(epoch, iter,
                                                                                                     n_cf_batch))
                    sys.exit()
            ...
            for iter in range(1, n_batch + 1):
                ...
                batch_loss = model(cf_batch_user, cf_batch_pos_item, cf_batch_neg_item, kg_batch_head,
                                   kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail, mode='train_cf&kg')

                if np.isnan(batch_loss.cpu().detach().numpy()):
                    logging.info(
                        'ERROR (CF&KG Training): Epoch {:04d} Iter {:04d} / {:04d} Loss is nan.'.format(epoch, iter,
                                                                                                        n_batch))
                    sys.exit()
                ...
                if (iter % args.cf_print_every) == 0:
                    logging.info(
                        'CF&KG Training: Epoch {:04d} Iter {:04d} / {:04d} | Time {:.1f}s | Iter Loss {:.4f} | Iter Mean Loss {:.4f}'.format(
                            epoch, iter, n_batch, time() - time6, batch_loss.item(), total_loss / iter))
            logging.info(
                'CF&KG Training: Epoch {:04d} Total Iter {:04d} | Total Time {:.1f}s | Iter Mean Loss {:.4f}'.format(
                    epoch, n_batch, time() - time5, total_loss / n_batch))
        else:
            logging.info('Unsupport train method {}'.format(train_method))
            sys.exit()
