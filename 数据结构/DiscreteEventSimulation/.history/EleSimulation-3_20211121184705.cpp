...
typedef struct Buildings{
    int CallUp[5][2];          
    int CallDown[5][2];     //分别对应每层楼的上升按钮和下降按钮是否被按亮,楼层共5层，编号0-4
}Buildings;
...
//判断电梯的下一个状态
State DecideNextState(int floor,int x);

//显示当前状态
void ShowStates(int x);  //显示第几个电梯的状态
void TableShowStates(char* SB,char* S,char* SN,int d0,int d1,int d2,int d3,int d4,int Up0,int Up1,int Up2,int Up3,int Up4,int Down0,int Down1,int Down2,int Down3,int Down4,int floor);

//随机数函数
void RandomItem(int *InFloor,int *OutFloor,int *GiveupTime,int *InterTime);    

//消除返回1楼事件，一旦电梯服务停止，启动时要保证消除返回1楼事件
void RemoveBackOne(EventList *elptr,int x);
...
void InitService(){
    ...
    for(i=0;i<2;i++){
    elevator[i].floor = 1;
    elevator[i].state = Idle;
    elevator[i].statebefore = UnKnown;
    elevator[i].statenext = UnKnown;
    elevator[i].D1 = 0;       //为0，除非人们正在上下电梯
    elevator[i].D2 = 1;        //为0，如果电梯已经在某层等待300以上
    elevator[i].D3 = 0;        //为0，除非电梯门在开着，但无人进出
    }

    InitEventList(&ev);
    for(j=0;j<2;j++){
    for(i=0;i<5;i++){
        elevator[j].CallCar[i]=0; 
        InitPasserStack(&(elevator[j].passerstack[i]));     
        }  //初始化两个电梯
    }
    for(i=0;i<5;i++){ 
        Building.CallDown[i][0]=0;
        Building.CallDown[i][1]=0;
        Building.CallUp[i][0]=0;
        Building.CallUp[i][1]=0;
        InitQuene(&queneUp[i]);
        InitQuene(&queneDown[i]);  //初始化楼层与乘客队列
...
void ShowEvent(Event entmp){
    ...
    switch(entmp.type){
        case PassAriv: printf("一位乘客从%d楼进入本服务系统\n",entmp.floor);ShowStates(0);ShowStates(1);break;
        case PassGetUpOver:printf("一位乘客从%d楼进入电梯\n",entmp.floor);ShowStates(0);ShowStates(1);break;   
        case PassGetDownOver:printf("一位乘客从%d楼离开电梯\n",entmp.floor);ShowStates(0);ShowStates(1);break;
        case PassGiveUp:printf("一位乘客从%d楼放弃乘坐电梯\n",entmp.floor);ShowStates(0);ShowStates(1);break;
       
        case EleArivStay:printf("%d号电梯到达并开始停留在%d楼\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleCheck:printf("%d号电梯在%d楼检测是否还有人上下电梯\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleOpen: printf("%d号电梯在%d楼开始开门\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleClose: printf("%d号电梯在%d楼开始关门\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleOpenOver:printf("%d号电梯在%d楼开门结束\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleCloseOver:printf("%d号电梯在%d楼关门结束\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;

        case EleStUp:printf("%d号电梯在%d楼开始上行\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleStDown:printf("%d号电梯在%d楼开始下行\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleBackOne:printf("%d号电梯在%d楼开始返回1层等候\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleStDownOneFloor:printf("%d号电梯下降1层，现位于%d层\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleStUpOneFloor:printf("%d号电梯上升1层，现位于%d层\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleBackOneDownOneFloor: printf("%d号电梯正返回1层，已到%d层\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;
        case EleBackOneUpOneFloor: printf("%d号电梯正返回1层，已到%d层\n",entmp.x,entmp.floor);ShowStates(0);ShowStates(1);break;

        default: break;
    }
}

void ShowStates(int x){
    
    char *SB,*S,*SN;
    int floor = elevator[x].floor;
    switch(elevator[x].statebefore){
        case UnKnown: SB = "UkON";break;
        case GoingUp: SB = "GoUp";break;
        case GoingDown: SB = "GoDn";break;
        case Idle: SB = "Idle";break;
    }
    switch(elevator[x].state){
        case UnKnown: S = "UkON";break;
        case GoingUp: S = "GoUp";break;
        case GoingDown: S = "GoDn";break;
        case Idle: S = "Idle";break;
    }
    switch(elevator[x].statenext){
    ...
    for(i=0;i<5;i++){
       ...
       Passenger * top = elevator[x].passerstack[i].top;
       while(top != (elevator[x].passerstack[i].base)){
           top -- ;
           d[i]++;
       }
    }
    TableShowStates(SB,S,SN,d[0],d[1],d[2],d[3],d[4],Up[0],Up[1],Up[2],Up[3],Up[4],Down[0],Down[1],Down[2],Down[3],Down[4],floor);
    printf("\n\n");
...
void HandlePassAriv(Event entmp){                                          
    //进入电梯，为下一人的出现做准备
    PriorTasks(entmp);
    elevator[0].statenext = DecideNextState(elevator[0].floor,0);
    elevator[1].statenext = DecideNextState(elevator[1].floor,1);
    Passenger passer;
    ...
    entmpNew1 = {entmp.OccurTime + InterTime,PassAriv,-1,-1};
    int x = GetElevator(passer);
    passer.x = x;
    entmpNew2 = {passer.GiveupTime,PassGiveUp,passer.infloor,x};
    ...
    if(passer.outfloor > passer.infloor)  {
         ...
         Building.CallUp[passer.infloor][x]=1;
    } 
    else {
        EnQuene(&queneDown[passer.infloor], passer);
        Building.CallDown[passer.infloor][x] =1;
    }
    
    //根据电梯状态，分为两种情况:电梯服务暂时性停止，电梯正在运行中,仅需对电梯服务暂时性停止情况进行处理
    if(elevator[x].statenext == UnKnown){
        if(elevator[x].floor==passer.infloor){
            if((elevator[x].D1==0)&&(elevator[x].D3==0)) //电梯在关着，此时电梯服务停止
            {
              entmpNew3 = {en.OccurTime,EleOpen,elevator[x].floor,x};
                OrderInsert(entmpNew3); 
                RemoveBackOne(&ev,x);
                elevator[x].statenext = DecideNextState(elevator[x].floor,x);
            }
            else if(elevator[x].D3==1){                      //门开着，无人
               entmpNew4 ={en.OccurTime+5,PassGetUpOver,elevator[x].floor,x} ;
            ...
            if((elevator[x].D1==0)&&(elevator[x].D3==0)){  //电梯在关着，此时电梯服务停止
                RemoveBackOne(&ev,x);
                if(entmp.floor > elevator[x].floor){
                   entmpNew3 = {entmp.OccurTime,EleStUp,elevator[x].floor,x};
                    OrderInsert(entmpNew3);
                    elevator[x].statenext = GoingUp;
                }
                else {
                entmpNew3 = {entmp.OccurTime,EleStDown,elevator[x].floor,x};
                OrderInsert(entmpNew3);  
                elevator[x].statenext = GoingDown;
                }
            }
        }
    }
        ...
        if(elevator[x].floor==passer.infloor){
            if(elevator[x].D3==1){       //电梯开着，无人上
                if(elevator[x].statenext == GoingUp){
                    if(passer.outfloor > passer.infloor){
                      entmpNew4 ={en.OccurTime+5,PassGetUpOver,elevator[x].floor,x} ;
                        OrderInsert(entmpNew4); 
                }
            }
                            if(elevator[x].statenext == GoingDown){
                    if(passer.outfloor < passer.infloor){
...
void HandlePassGiveUp(Event entmp){           //预置事件，需先判断,此处只需队列中有需离开的让其离开即可
  ...
  int x =entmp.x;
    if(entmp.type == PassGiveUp){
        if(elevator[x].floor!=entmp.floor){
...
void HandlePassGetUpOver(Event entmp){
   PriorTasks(entmp);
    ...
    if(elevator[x].statenext == GoingUp){
        if(!EmptyQuene(queneUp[entmp.floor]))  {
            ...
            DeQuene(&queneUp[entmp.floor],&passertmp);
             StackPush(&elevator[x].passerstack[passertmp.outfloor],passertmp);
              elevator[x].CallCar[passertmp.outfloor] = 1;
                 if(!EmptyQuene(queneUp[entmp.floor])){
                  entmpNew = {entmp.OccurTime+5,PassGetUpOver,entmp.floor,x};
    ...
    else if(elevator[x].statenext==GoingDown){
      if(!EmptyQuene(queneDown[entmp.floor])){
            ...
            elevator[x].statenext = DecideNextState(elevator[x].floor,x);
             ...
             StackPush(&elevator[x].passerstack[passertmp.outfloor],passertmp);        //访问栈时为空
              elevator[x].CallCar[passertmp.outfloor] = 1;
                        if(!EmptyQuene(queneDown[entmp.floor])){
                 entmpNew = {entmp.OccurTime+5,PassGetUpOver,entmp.floor,x};
                  OrderInsert(entmpNew);
...
void HandlePassGetDownOver(Event entmp){
    Passenger passertmp;
    int x = entmp.x;
    StackPop(&(elevator[x].passerstack[entmp.floor]),&passertmp);
    ShowEvent(entmp);     //显示事件处理情况
}
...
int x = entmp.x;
    if(entmp.type == EleArivStay){
        elevator[x].statebefore = elevator[x].state;
        elevator[x].state = elevator[x].statenext; 
        elevator[x].statenext = DecideNextState(elevator[x].floor,x);       
        elevator[x].floor = entmp.floor;
        Event entmpNew = {entmp.OccurTime,EleOpen,entmp.floor,x};
        OrderInsert(entmpNew);
        ShowEvent(entmp);
    }
}     
void HandleEleOpen(Event entmp){
    ...
    if(entmp.type == EleOpen){
        Event entmpNew = {entmp.OccurTime+5,EleOpenOver,entmp.floor,x};
        OrderInsert(entmpNew);
        ShowEvent(entmp);
    }
} 
void HandleEleOpenOver(Event entmp){  
    ...
    if(entmp.type == EleOpenOver){
        ...
        entmpNew1 =   {entmp.OccurTime+10,EleCheck,entmp.floor,x}; 
        OrderInsert(entmpNew1);
        if(elevator[x].CallCar[elevator[x].floor] == 1){
          elevator[x].D1 = 1;
            elevator[x].D3 = 0;
            elevator[x].statenext = DecideNextState(elevator[x].floor,x);
            top = elevator[x].passerstack[entmp.floor].top;
            base = elevator[x].passerstack[entmp.floor].base;
            while(top!=base){
                entmpNew2 = {Time+5,PassGetDownOver,entmp.floor,x};
                OrderInsert(entmpNew2);
                Time+=5;
                top--;
        }
    ...
    elevator[x].statenext = DecideNextState(elevator[x].floor,x);
        if(elevator[x].statenext== GoingUp) {
            if(!EmptyQuene(queneUp[entmp.floor])){
               entmpNew3 = {Time+5,PassGetUpOver,entmp.floor,x};
                OrderInsert(entmpNew3);
                elevator[x].D3 = 0;
                elevator[x].D1 = 1;
                elevator[x].statenext = DecideNextState(elevator[x].floor,x);
...
void HandleEleCheck(Event entmp){                      //检查中断
    ...
    if(elevator[x].statenext == GoingUp){
        if(EmptyQuene(queneUp[entmp.floor])){
            entmpNew = {entmp.OccurTime,EleClose,entmp.floor,x};
            OrderInsert(entmpNew);
        }
        else {
        entmpNew = {entmp.OccurTime+10,EleCheck,entmp.floor,x};
        OrderInsert(entmpNew);
    }
    }
    else if(elevator[x].statenext == GoingDown){
...
void HandleEleClose(Event entmp){
    ...
    elevator[x].D2 = 1;
    elevator[x].statenext = DecideNextState(elevator[x].floor,x);
    Event entmpNew ={entmp.OccurTime+5,EleCloseOver,entmp.floor,x};
    OrderInsert(entmpNew);
    ShowEvent(entmp);
}
void HandleEleCloseOver(Event entmp){
 ...
 Event entmpNew;
    ...
    if(elevator[x].statenext == GoingUp){
        if(!EmptyQuene(queneUp[entmp.floor])){
          entmpNew = {entmp.OccurTime,EleOpen,entmp.floor,x};
            OrderInsert(entmpNew);
        }
        else{
          entmpNew = {entmp.OccurTime,EleStUp,entmp.floor,x};
            OrderInsert(entmpNew);
        } 
    }
    else if(elevator[x].statenext == GoingDown){
        if(!EmptyQuene(queneDown[entmp.floor])){
            ...
            OrderInsert(entmpNew);
             ...
             entmpNew = {entmp.OccurTime,EleStDown,entmp.floor,x};
            OrderInsert(entmpNew);
        }
    }
    else {
        Event entmpNew = {entmp.OccurTime+100,EleBackOne,entmp.floor,x};
        OrderInsert(entmpNew);
    }
    ShowEvent(entmp);
}
void HandleEleStUp(Event entmp){  
    ...
    elevator[x].state = elevator[x].statenext; 
    elevator[x].statenext = DecideNextState(elevator[x].floor,x);
    elevator[x].CallCar[entmp.floor] = 0;
    Building.CallUp[entmp.floor][x] = 0;
    Event entmpNew = {entmp.OccurTime+10,EleStUpOneFloor,entmp.floor+1,x};
    OrderInsert(entmpNew);
    ShowEvent(entmp);
}  
void HandleEleStUpOneFloor(Event entmp){
  ...
  Event entmpNew;
    ...
    if(floortmp==entmp.floor){
       entmpNew = {entmp.OccurTime+2,EleArivStay,floortmp,x};
        OrderInsert(entmpNew);
    }
    else if(floortmp >= 0){
         elevator[x].floor = entmp.floor;
        ShowEvent(entmp);
        entmpNew = {entmp.OccurTime+10,EleStUpOneFloor,entmp.floor+1,x};
        OrderInsert(entmpNew);
    }
...
void HandleEleStDown(Event entmp){
    ...
    elevator[x].state = elevator[x].statenext; 
    elevator[x].statenext = DecideNextState(elevator[x].floor,x);
    elevator[x].CallCar[entmp.floor] = 0;Building.CallDown[entmp.floor][x] = 0;
    Event entmpNew = {entmp.OccurTime+12,EleStDownOneFloor,entmp.floor-1,x};
    OrderInsert(entmpNew);
    ShowEvent(entmp);
}   
void HandleEleStDownOneFloor(Event entmp){
 ...
 PriorTasks(entmp);
    ...
    int floortmp = GetNextFloor(entmp);
    if(floortmp==entmp.floor){
       entmpNew = {entmp.OccurTime+4,EleArivStay,floortmp,x};
        OrderInsert(entmpNew);
    }
    else if(floortmp >=0 ){
         elevator[x].floor = entmp.floor;
            ShowEvent(entmp);
       entmpNew = {entmp.OccurTime+12,EleStDownOneFloor,entmp.floor-1,x};
        OrderInsert(entmpNew);
    }
...
void HandleEleBackOne(Event entmp){                //此过程要保证此时先将到达事件处理
   PriorTasks(entmp);
    ...
    int x = entmp.x;
    elevator[x].statenext = DecideNextState(elevator[x].floor,x);  
    if(elevator[x].statenext == UnKnown){      //此时电梯处于闲置，且仍然无具体活动，即将返回
            if(entmp.floor > 1) {
               entmpNew = {entmp.OccurTime+ 3+12,EleBackOneDownOneFloor,entmp.floor-1,x};
                OrderInsert(entmpNew); 
    }
            if(entmp.floor < 1) {
                entmpNew = {entmp.OccurTime+ 2+10,EleBackOneUpOneFloor,entmp.floor+1,x};
                OrderInsert(entmpNew); 
    }
    ShowEvent(entmp);
    }    
...
void HandleEleBackOneDownOneFloor(Event entmp){
 ...
 int x = entmp.x;
    int floortmp = GetNextFloor(entmp);
    if(floortmp==entmp.floor){
     entmpNew = {entmp.OccurTime+3,EleArivStay,floortmp,x};
        OrderInsert(entmpNew);
    }
    else if(entmp.floor!=1){
        elevator[x].floor = entmp.floor;
        ShowEvent(entmp);
       entmpNew = {entmp.OccurTime+12,EleBackOneDownOneFloor,entmp.floor-1,x};
        OrderInsert(entmpNew);
    }
 ...
 int x = entmp.x;
  Event entmpNew;
    ...
    if(floortmp==entmp.floor){
        ...
        elevator[x].floor = entmp.floor;
        ShowEvent(entmp);
        entmpNew = {entmp.OccurTime+10,EleBackOneUpOneFloor,entmp.floor+1,x};
        OrderInsert(entmpNew);
    }
...
//获取电梯的下一个状态
State DecideNextState(int floor,int x){            //此函数处理电梯处于激活状态，非激活状态特殊考虑 
    switch(elevator[x].state){
        case GoingUp : return Idle;break;     
        case GoingDown: return Idle;break; 
        case Idle: 
            int i1,i2;
            if(elevator[x].statebefore==GoingUp){
                for(i1=floor+1;i1<5;i1++){
                    if(elevator[x].CallCar[i1]==1) return  GoingUp;
                } 
                 for(i1=floor+1;i1<5;i1++){
                    if(Building.CallDown[i1][x] ||Building.CallUp[i1][x]) return  GoingUp;
                }
                if(Building.CallUp[floor][x]) return GoingUp;
                if(Building.CallDown[floor][x]) return GoingDown;
            }
            if(elevator[x].statebefore==GoingDown){
                for(i2=floor-1;i2>=0;i2--){
                    if(elevator[x].CallCar[i2]==1)  return GoingDown;
                } 
                 for(i2=floor-1;i2>=0;i2--){
                    if(Building.CallDown[i2][x] ||Building.CallUp[i2][x]) return  GoingDown;
                }
    ...
    switch(elevator[x].state){
        case GoingDown:
                    for(i1=entmp.floor;i1>=0;i1--){
                        if(elevator[x].CallCar[i1])  break;
                    }
                    for(i2=entmp.floor;i2>=0;i2--){
                        if(Building.CallDown[i2][x]) break;
                    }
                    if((i1>=0)&&(i2>=0)){
                      return   (i1>i2? i1:i2);
                    }
        ...
        case GoingUp:
                    for(i1=entmp.floor;i1<5;i1++){
                        if(elevator[x].CallCar[i1]==1)  break;
                    }
                    for(i2=entmp.floor;i2<5;i2++){
                        if(Building.CallUp[i2][x]) 
                            break;
                    }
                    if((i1<5)&&(i2<5)){
                      return (i1<i2? i1:i2);
...
//消除返回1楼事件，一旦电梯服务停止，启动时要保证消除返回1楼事件
void RemoveBackOne(EventList *EListptr,int x ){
        EventNode * evtmp = *EListptr;
        EventNode * evtmpnext;
        while((evtmp->next)&&(evtmp->next->event.type != EleBackOne)&&(!(evtmp->next->event.x == x))){
            evtmp = evtmp->next;
        }
        if((evtmp->next)&&(evtmp->next->event.type == EleBackOne)&&(evtmp->next->event.x == x)){
...
void TableShowStates(char* SB,char* S,char* SN,int d0,int d1,int d2,int d3,int d4,int Up0,int Up1,int Up2,int Up3,int Up4,int Down0,int Down1,int Down2,int Down3,int Down4,int floor,int x){
putchar('|');
putchar('<');
putchar('-');
putchar('-');
