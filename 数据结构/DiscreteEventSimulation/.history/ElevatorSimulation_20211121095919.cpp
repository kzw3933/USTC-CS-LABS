...
void HandlePassAriv(Event entmp){                                          //出错
    ...
    if(elevator.statenext == UnKnown){
        if(elevator.floor==passer.infloor){
            ...
            {
                 Event entmpNew3 = {en.OccurTime,EleOpen,elevator.floor};
                OrderInsert(entmpNew3); 
                RemoveBackOne(&ev);
                elevator.statenext = DecideNextState(elevator.floor);
            }
            else if(elevator.D3==1){                      //门开着，无人
                Event entmpNew4 ={en.OccurTime+25,PassGetUpOver,elevator.floor} ;
            ...
            if((elevator.D1==0)&&(elevator.D3==0)){  //电梯在关着，此时电梯服务停止
                ...
                if(entmp.floor > elevator.floor){
                    Event entmpNew3 = {entmp.OccurTime,EleStUp,elevator.floor};
                    OrderInsert(entmpNew3);
                    elevator.statenext = GoingUp;
                }
                else {
                Event entmpNew3 = {entmp.OccurTime,EleStDown,elevator.floor};
                OrderInsert(entmpNew3);  
            ...
            DeQuene(&queneUp[entmp.floor],&passertmp);
             StackPush(&elevator.passerstack[passertmp.outfloor],passertmp);
              elevator.CallCar[passertmp.outfloor] = 1;
                 if(!EmptyQuene(queneUp[entmp.floor])){
                  Event entmpNew = {entmp.OccurTime+25,PassGetUpOver,entmp.floor};
...
void HandleEleOpenOver(Event entmp){   
    ...
    if(entmp.type == EleOpenOver){
        ...
        entmpNew1 =   {entmp.OccurTime+40,EleCheck,entmp.floor}; 
        OrderInsert(entmpNew1);
        if(elevator.CallCar[elevator.floor] == 1){
          elevator.D1 = 1;
            ...
            while(top!=base){
                entmpNew2 = {Time+25,PassGetDownOver,entmp.floor};
                OrderInsert(entmpNew2);
                Time+=25;
                top--;
        }
    ...
    elevator.statenext = DecideNextState(elevator.floor);
        if(elevator.statenext== GoingUp) {
            if(!EmptyQuene(queneUp[entmp.floor])){
               entmpNew3 = {Time+25,PassGetUpOver,entmp.floor};
                OrderInsert(entmpNew3);
                elevator.D3 = 1;
                elevator.D1 = 0;
            }
...
void HandleEleCheck(Event entmp){                      //检查中断
    ...
    if(elevator.statenext == GoingUp){
        ...
        entmpNew = {entmp.OccurTime+40,EleCheck,entmp.floor};
        OrderInsert(entmpNew);
    }
    }
    else if(elevator.statenext == GoingDown){
...
void HandleEleStUpOneFloor(Event entmp){
  ...
  Event entmpNew;
    ...
    if(floortmp==entmp.floor){
       entmpNew = {entmp.OccurTime+14,EleArivStay,floortmp};
        OrderInsert(entmpNew);
    }
    else {
         elevator.floor = entmp.floor;
        ShowEvent(entmp);
        entmpNew = {entmp.OccurTime+51,EleStUpOneFloor,entmp.floor+1};
        OrderInsert(entmpNew);
    }
...
void HandleEleStDownOneFloor(Event entmp){
 ...
 PriorTasks(entmp);
    ...
    int floortmp = GetNextFloor(entmp);
    if(floortmp==entmp.floor){
       entmpNew = {entmp.OccurTime+23,EleArivStay,floortmp};
        OrderInsert(entmpNew);
    }
    else {
         elevator.floor = entmp.floor;
            ShowEvent(entmp);
       entmpNew = {entmp.OccurTime+61,EleStDownOneFloor,entmp.floor-1};
        OrderInsert(entmpNew);
    }
...
void HandleEleBackOne(Event entmp){                //此过程要保证此时先将到达事件处理
   PriorTasks(entmp);
    ...
    if(elevator.statenext == UnKnown){      //此时电梯处于闲置，且仍然无具体活动，即将返回
            if(entmp.floor > 1) {
               entmpNew = {entmp.OccurTime+ 15+61,EleBackOneDownOneFloor,entmp.floor-1};
                OrderInsert(entmpNew); 
    }
            if(entmp.floor < 1) {
                entmpNew = {entmp.OccurTime+ 15+51,EleBackOneUpOneFloor,entmp.floor+1};
                OrderInsert(entmpNew); 
    }
    ShowEvent(entmp);
    }    
 ...
 Event entmpNew;
    ...
    if(floortmp==entmp.floor){
     entmpNew = {entmp.OccurTime+23,EleArivStay,floortmp};
        ...
        elevator.floor = entmp.floor;
        ShowEvent(entmp);
       entmpNew = {entmp.OccurTime+61,EleBackOneDownOneFloor,entmp.floor-1};
        OrderInsert(entmpNew);
    }
 ...
 PriorTasks(entmp);
  Event entmpNew;
    ...
    if(floortmp==entmp.floor){
        ...
        elevator.floor = entmp.floor;
        ShowEvent(entmp);
        entmpNew = {entmp.OccurTime+51,EleBackOneUpOneFloor,entmp.floor+1};
        OrderInsert(entmpNew);
    }
