...
void PrintPolyn(PolynList P ){             //打印多项式
	struct PolynNode *Pa=P->next;
    if(P->expn==1 && Pa->coef == 0)  printf("%.1f",Pa->coef);
    else {
        while(Pa){
...
void DeleteZeroPolyn(PolynList P){
    ...
    while(Pa&&Pa->next){
        ...
        else if((Pa->next->coef == 0) && (P->expn == 1)){
            Pa->next->expn = 0;
        }
        Pa = Pa->next;
    }
...
// 1.指针的指针：值传递的理解,不传入未初始化的指针   
// 2.向空赋值：导致程序终止

// 3.功能分离：模块化设计，否则，不同功能同时实现之间关系、情况太多
// 4.指针偏移：可能指向空指针