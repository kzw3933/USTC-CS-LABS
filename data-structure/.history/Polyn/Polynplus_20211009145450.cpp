...
void MultiplyPolyn(PolynList Pa,PolynList Pb)
{
    ...
    while(ha->next)
    {
        ...
        while(hb->next)
        {
            NewNodeptr = (PolynNode *)malloc(sizeof(struct PolynNode));NewNode->next = NULL;
            hb=hb->next; NewNode->coef=ha->coef * hb->coef;NewNode->expn = ha->expn + hb->expn;
