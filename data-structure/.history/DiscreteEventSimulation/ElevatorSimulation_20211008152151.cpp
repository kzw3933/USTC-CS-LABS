...
#include<stdlib.h>
//定义必要的数据类型
typedef  enum EventType {EleStOffPre,EleAriv,EleStOff,PassAriv,PassDepart,PassGiveUp,PassGiveUpPre} EventType;
...
void HandleEleStartOff(Event entmp);
...
void HandleCurEvent(Event entmp){
        ...
        case PassDepart: HandlePassDepart(entmp);break;
        case PassGiveUp: HandlePassGiveUp(entmp);break;
        case EleAriv:HandleEleAriv(entmp);break;
        case EleStOff: HandleEleStartOff(entmp);break;
        default: break;
    }  
}
...
void HandlePassAriv(Event entmp){
    ...
    if(passer.infloor == elevator.floor){
        if(elevator.state == Idle){
            ...
            elevator.D1 = 1; 
            Event entmpNew3 ;
            entmpNew3.OccurTime = en.OccurTime + 65;          //开关门各需20，每人进出各需25
            entmpNew3.type = EleStOffPre;
