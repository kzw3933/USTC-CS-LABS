...
//也就是说事件具有不同的优先度,对于同时刻发生的事,要保证: 1. 电梯到达停留，乘客到达,先处理电梯到达停留 2.电梯到达停留，乘客放弃,先处理电梯到达停留 3. 电梯检测，乘客到达，先处理乘客到达  4.电梯关闭结束，乘客到达，先处理乘客到达 5.电梯即将返回1楼等待,乘客到达，先处理乘客到达   6.乘客上电梯结束，另一个乘客到达，先处理乘客到达
...
void HandlePassGetDownOver(Event entmp){
    ...
    if(!EmptyStack(elevator.passerstack[entmp.floor])){
         Event entmpNew = {entmp.OccurTime+25,PassGetDownOver,entmp.floor};
         OrderInsert(entmpNew);
    }
    ShowEvent(entmp);     //显示事件处理情况
}
