...
void ShowEvent(Event entmp){
    ...
    switch(entmp.type){
        ...
        case EleBackOneDownOneFloor: printf("电梯已返回%d层\n",entmp.floor);break;
        case EleBackOneUpOneFloor: printf("电梯已返回%d层\n",entmp.floor);break;
...
void HandlePassAriv(Event entmp){
    ...
    entmpNew2.OccurTime = entmp.OccurTime+passer.GiveupTime;
    ...
    if(passer.outfloor > passer.infloor){
        if(EmptyQuene(queneUp[passer.infloor]))  signal = 1;
        EnQuene(&queneUp[passer.infloor], passer); 
    }
           
    else {
        if(EmptyQuene(queneDown[passer.infloor]))  signal = 1;
    ...
    if(elevator.statenext == UnKnown){
        if(elevator.floor==passer.infloor){
            if((elevator.D1==0)&&(elevator.D3==0))
            {
                ...
                if(passer.outfloor>passer.infloor)  elevator.statenext = GoingUp;   //电梯状态的激活
                else elevator.statenext = GoingDown;
                if(signal){
                Event entmpNew4 ;
...
void HandlePassGetUpOver(Event entmp){
    ...
    else DeQuene(&queneDown[entmp.floor],&passertmp);  
...
void HandleEleOpenOver(Event entmp){   
    ...
    if(entmp.type == EleOpenOver){
        ...
        if(elevator.statenext== GoingUp) {
            ...
            else if(!EmptyQuene(queneDown[entmp.floor])){
                    Event entmpNew3 = {Time+25,PassGetUpOver,entmp.floor};
                    OrderInsert(entmpNew3);
                    elevator.D3 = 1;
                    elevator.D1 = 0;    
        ...
        if(elevator.statenext== GoingDown) {
            ...
            else  if(!EmptyQuene(queneUp[entmp.floor])){
                Event entmpNew3 = {Time+25,PassGetUpOver,entmp.floor};
                OrderInsert(entmpNew3);
                elevator.D3 = 1;
                elevator.D1 = 0;
...
void HandleEleStDown(Event entmp){
    ...
    Event entmpNew = {entmp.OccurTime+15+61,EleStDownOneFloor,entmp.floor+1};
    OrderInsert(entmpNew);
    ShowEvent(entmp);
}   
void HandleEleStDownOneFloor(Event entmp){
...
void HandleEleBackOne(Event entmp){                //预置事件，需先判断
    ...
    if(elevator.statenext == UnKnown){
            ...
            if(entmp.floor < 1) {
                Event entmpNew = {entmp.OccurTime+ 15+51,EleBackOneDownOneFloor,entmp.floor+1};
                OrderInsert(entmpNew); 
    }
    ShowEvent(entmp);
    }    
    ...
    switch(elevator.state){
        case GoingDown:
                    ...
                    if((i1>=0)&&(i2>=0)){
                        ...
                        for(i3=entmp.floor-1;i3>=0;i3--){
                        if(!(EmptyQuene(queneUp[i2]))) 
                            return i3;
                    }
                    }
                    return -1;
