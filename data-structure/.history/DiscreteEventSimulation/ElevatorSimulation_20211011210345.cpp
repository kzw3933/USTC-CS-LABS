...
#include<stdlib.h>
//定义必要的数据类型
typedef  enum EventType {EleStay,EleCheck,EleArivStay,EleBackOne,EleOpen,EleClose,EleOpenOver,EleCloseOver,EleStUp,EleStDown,PassAriv,PassGetUp,PassGetDown,PassGiveUp} EventType;
//事件具有不同的优先度,对于同时刻发生的事,要保证 1. 电梯到达停留，乘客到达,先处理电梯到达停留 2.电梯到达停留，乘客放弃,先处理电梯到达停留 3. 电梯检测，乘客到达，先处理乘客到达  4.电梯关闭结束，乘客到达，先处理乘客到达 5.电梯即将返回1楼等待,乘客到达，先处理乘客到达
...
void HandleCurEvent(Event entmp){
        ...
        case PassGetUpOver: HandlePassGetUp(entmp);break;
        case PassGetDownOver:HandlePassGetDown(entmp);break;
        case PassGiveUp: HandlePassGiveUp(entmp);break;
    
        case EleArivStay:HandleEleAriv(entmp);break;
...
void HandleEleBackOne(Event entmp){                //预置事件，需先判断
    ...
    elevator.statenext = DecideNextState(entmp.floor);
    if(entmp.floor > 1) {
        Event entmpNew = {entmp.OccurTime+ 15+(entmp.floor-1)*61+23,EleStay,1};
        OrderInsert(entmpNew); 
    }
    if(entmp.floor < 1) {
        Event entmpNew = {entmp.OccurTime+ 15+(1-entmp.floor)*51+14,EleStay,1};
        OrderInsert(entmpNew); 
    }
    }
...
//对时间表进行合理的排序
void RegularEventList(EventList *elptr){
