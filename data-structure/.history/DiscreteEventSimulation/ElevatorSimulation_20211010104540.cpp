...
void InitService(){
    ...
    InitEventList(&ev);
    InitPasserStack(&elevator.passerstack);
    //各层及其队列初始化，电梯初始化
    for(int i=0;i<5;i++){
...
void HandlePassGiveUp(Event entmp){           //优先级最低,此时无其他事件同时发生
        if(entmp.type == PassGiveUp) {
            DelQueneNode(&q[entmp.floor],entmp.OccurTime);
        } 
}
void HandlePassGetUpOver(Event entmp){
    Passenger passertmp;
    DelQuene(&q[entmp.floor],&passertmp);
    StackPush(&elevator.passerstack,passertmp);
}
void HandlePassGetDownOver(Event entmp){
    Passenger passertmp;
    StackPop(&elevator.passerstack,&passertmp);
...
void HandleEleAriv(Event entmp){
    if(entmp.type == EleAriv){
        ...
        elevator.state = Idle;
        Event entmpNew = {entmp.OccurTime,entmp.floor,EleOpen};
    }
