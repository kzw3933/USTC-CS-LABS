...
void OrderInsert(EventList ev,Event entmp){
    EventNode* temp1 = ev;
    EventNode* temp2 = (EventNode*)malloc(sizeof(EventNode));
    temp2->event = entmp;
    temp2->next =NULL;
    if(temp1->next){
        while(temp1->next->event.Occurtime <entmp.Occurtime) temp1 = temp1->next;  //找到插入的位置，按时间升序
    }
...
void Random(int* durtime,int* money,int* type,int* intertime){
    *durtime = rand()%20+5;    //默认处理一件事务不超过20分钟,最少5分钟
    *money = rand() %2000 +200;   //默认处理事务的钱数不超过2000，最少200;
    *type = rand() %2;   //
    *intertime = rand()%20 +5; // 两人来的间隔至少5分钟
    printf("%d %d %d %d \n",*durtime,*money,*type,*intertime);
}
void GetQHead(QueneList q,Customer *customerptr){
    *customerptr = q.rear->next->customer;
}