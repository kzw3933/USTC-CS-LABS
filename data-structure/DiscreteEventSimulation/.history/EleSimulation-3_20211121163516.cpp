...
int main(){
    printf("-----------------------------电梯模拟服务即将开始----------------------------\n");
    InitService();               //初始化服务
    elevator[x]Simulation();        //电梯模拟服务开始
    CloseService();              //服务停止
}
//流程相关函数
void InitService(){
    int i;int j;
    for(i=0;i<2;i++){
    elevator[x][i].floor = 1;
    elevator[x][i].state = Idle;
    elevator[x][i].statebefore = UnKnown;
    elevator[x][i].statenext = UnKnown;
    elevator[x][i].D1 = 0;       //为0，除非人们正在上下电梯
    elevator[x][i].D2 = 1;        //为0，如果电梯已经在某层等待300以上
    elevator[x][i].D3 = 0;        //为0，除非电梯门在开着，但无人进出
    }

    InitEventList(&ev);
    for(j=0;j<2;j++){
    for(i=0;i<5;i++){
        elevator[x][j].CallCar[i]=0; 
        InitPasserStack(&(elevator[x][j].passerstack[i]));     
        }  //初始化两个电梯
    }
      for(i=0;i<5;i++){
        elevator[x][j].CallCar[i]=0; 
...
void ShowStates(int x){
    ...
    for(i=0;i<5;i++){
       ...
       Passenger * top = elevator[x][x].passerstack[i].top;
       while(top != (elevator[x][x].passerstack[i].base)){
           top -- ;
           d[i]++;
       }
    }
...
void HandlePassAriv(Event entmp){                                          
    //进入电梯，为下一人的出现做准备
    PriorTasks(entmp);
    elevator[x][0].statenext = DecideNextState(elevator[x][0].floor);
    elevator[x][1].statenext = DecideNextState(elevator[x][1].floor);
    Passenger passer;
    ...
    int x= Getelevator[x]();
    if(elevator[x][x].statenext == UnKnown){
        if(elevator[x][x].floor==passer.infloor){
            if((elevator[x][x].D1==0)&&(elevator[x][x].D3==0)) //电梯在关着，此时电梯服务停止
            {
              entmpNew3 = {en.OccurTime,EleOpen,elevator[x][x].floor};
                OrderInsert(entmpNew3); 
                RemoveBackOne(&ev);
                elevator[x][x].statenext = DecideNextState(elevator[x][x].floor);
            }
            else if(elevator[x][x].D3==1){                      //门开着，无人
               entmpNew4 ={en.OccurTime+5,PassGetUpOver,elevator[x][x].floor} ;
                OrderInsert(entmpNew4); 
            }

        }
        else {
            if((elevator[x][x].D1==0)&&(elevator[x].D3==0)){  //电梯在关着，此时电梯服务停止
                RemoveBackOne(&ev);
                if(entmp.floor > elevator[x].floor){
                   entmpNew3 = {entmp.OccurTime,EleStUp,elevator[x].floor};
                    OrderInsert(entmpNew3);
                    elevator[x].statenext = GoingUp;
                }
                else {
                entmpNew3 = {entmp.OccurTime,EleStDown,elevator[x].floor};
                OrderInsert(entmpNew3);  
        ...
        if(elevator[x].floor==passer.infloor){
            if(elevator[x].D3==1){       //电梯开着，无人上
                if(elevator[x].statenext == GoingUp){
                    if(passer.outfloor > passer.infloor){
                      entmpNew4 ={en.OccurTime+5,PassGetUpOver,elevator[x].floor} ;
                        OrderInsert(entmpNew4); 
                }
            }
...
void HandlePassGetUpOver(Event entmp){
   PriorTasks(entmp);
    Passenger passertmp;
    Event entmpNew ;
    elevator[x].statenext = DecideNextState(elevator[x].floor);
    if(elevator[x].statenext == GoingUp){
