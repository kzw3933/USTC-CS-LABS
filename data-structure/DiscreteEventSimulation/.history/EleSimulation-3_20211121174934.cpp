...
void HandlePassAriv(Event entmp){                                          
    //进入电梯，为下一人的出现做准备
    PriorTasks(entmp);
    int x = GetElevator(entmp);
    elevator[0].statenext = DecideNextState(elevator[x].floor,x);
    Passenger passer;
    ...
    entmpNew2 = {passer.GiveupTime,PassGiveUp,passer.infloor,-1};
...
//int GetElevator(Event entmp){
    int m = elevator[0].floor;
    int n = elevator[1].floor;
    int j = entmp.floor;
