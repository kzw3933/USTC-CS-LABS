...
void memTestCaseInit(void){	
	addNewCmd("testMalloc1\0", testCase1, 0, "Malloc, write and read.\0");
	addNewCmd("testMalloc2\0", testCase2, 0, "Malloc, write and read.\0");		
	addNewCmd("maxMallocSizeNow\0", maxMallocSizeNow, 0, "MAX_MALLOC_SIZE always changes. What's the value Now?\0");

	addNewCmd("testdP1\0", testdP1, 0, "Init a dPatition(size=0x100) [Alloc,Free]* with step = 0x20\0");
	addNewCmd("testdP2\0", testdP2, 0, "Init a dPatition(size=0x100) A:B:C:- ==> -:B:C:- ==> -:C:- ==> - .\0");
	addNewCmd("testdP3\0", testdP3, 0, "Init a dPatition(size=0x100) A:B:C:- ==> A:B:- ==> A:- ==> - .\0");
	
	addNewCmd("testeFP\0", testeFP, 0, "Init a eFPatition. Alloc all and Free all.\0");
}
